...
*   **制御**: ユーザーがEnterキーを押すと、Arduinoに再開の信号を返します。
...
## 使い方
1.  デバイスを起動し、PCでPythonスクリプトを走らせます。（PC不要のスタンドアロン動作も可能ですが、スプレッドシート記録やPC通知には必要です）
2.  作業を開始するとArduinoの緑LEDが点灯し、カウントダウンが始まります。（離席すると中断）
3.  20分経過すると、PCに通知が届き、Arduinoが休憩モード（20秒）に入ります。
4.  20秒経過後、超音波センサーに手をかざす（15cm以内で1秒間）か、PC側でEnterキーを押すと、再び次のサイクルが始まります。
